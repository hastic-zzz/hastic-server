...
class GeneralModel(Model):
    ...
    def do_fit(self, dataframe: pd.DataFrame, labeled_segments: list, deleted_segments: list, learning_info: dict, id: AnalyticUnitId) -> None:
        ...
        for segment in deleted_segments:
            ...
            deleted_pat = utils.get_interval(data, del_mid_index, half_window_size)
        ...
        self.state['convolve_min'], self.state['convolve_max'] = utils.get_min_max(convolve_list, half_window_size / 3)
        self.state['conv_del_min'], self.state['conv_del_max'] = utils.get_min_max(del_conv_list, half_window_size)
    ...
    def do_detect(self, dataframe: pd.DataFrame, id: AnalyticUnitId) -> List[int]:
        ...
        half_window_size = self.state.get('WINDOW_SIZE', 0) // 2
        window_size = self.state.get('WINDOW_SIZE', 0)
        all_corr = utils.get_correlation_gen(data, half_window_size, pat_data)
        all_corr_peaks = utils.find_peaks(all_corr, window_size)
        ...
        return [(item, item + window_size) for item in filtered]
